...
const main = async () => {
    ...
    const wethAdress = "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2";    //wETH contract address

    const UNIRouter = "0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D";

    const USDCHolder = "0xf584f8728b874a6a5c7a8d4d387c9aae9172d621";    // //USDC holder address
    ...
    const transferTX = await ROUTER.connect(impersonatedSigner).swapExactTokensForTokens(
        amountOut,
        1000
        ,
    ...
    const usdcBalAfterTransfer = await USDC.balanceOf(impersonatedSigner.address);
    const daiBalAfterTransfer = await DAI.balanceOf(impersonatedSigner.address);

    console.log("-----------------------------------------------------------------")

    
    console.log("usdc balance after transfer", ethers.formatUnits(usdcBalAfterTransfer, 6) );
    console.log("dai balance after transfer", ethers.formatUnits(daiBalAfterTransfer, 18) );
